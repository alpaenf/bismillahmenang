...
export async function POST(request: Request) {
  try {
    ...
    const { messages, sessionTitle } = body;
    ...
    const flaggedMessages: FlaggedBubbleResult[] = [];
    let hasApk = false;
    let hasOtp = false;
    let hasReward = false;
    ...
    const escalationFlow: EscalationPhase[] = [
      {
        phaseNumber: 1,
    ...
    // SIMPAN HASIL ANALISIS WHATSAPP KE SUPABASE
    ...
    const responsePayload: WhatsAppAnalysisResponse = {
      ...
      indicators,
      recommendations,
      disclaimer: 'Analisis percakapan diproses secara otomatis dan tersimpan aman di database Supabase.',
    };
